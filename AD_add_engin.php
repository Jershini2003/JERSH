<?php
/* CREATE TABLE IF NOT EXISTS engincollege (
...
if (isset($_POST['link_url']) && isset($_POST['link_title'])) {
  ...
  if (move_uploaded_file($_FILES["link_image"]["tmp_name"], $target_file)) {
    ...
    $stmt = $conn->prepare("INSERT INTO engincollege (url, title,address, image_path) VALUES (?,?,?,?)");
    $stmt->bind_param("ssss", $link_url, $link_title,$link_address,$image_path);
    if ($stmt->execute()) {
      header("location:AD_add_engin.html");
    } else {
