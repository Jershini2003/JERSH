...
<head>
    ...
    <title>USER</title>
    <link rel="stylesheet" href="artscollege.css">
</head>
<body>
<div class="arts"><p>ARTS AND SCIENCE COLLEGES<p></div>
<?php
 require('server.php');

// Prepare and execute SQL statement to select all data from the "links" table
$sql = "SELECT title,address,url, image_path FROM links ORDER BY title ASC";
  ...
  while($row = $result->fetch_assoc()) {
    ...
    foreach($row as $key => $value) {
      if ($key == "image_path") {
       ...
       // echo '<div class="column">';
        //echo '<span class="column-name">' . $key . ': </span>';
        //echo '<span class="column-value">' . $value . '</span>';
       // echo '</div>';
      }
    }
    echo '</div>';
